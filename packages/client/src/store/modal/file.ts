...
export const useFileStore = defineStore('file', {
  ...
  actions: {
    ...
    addRecentFile(filePath: string, knowledgeBasePath?: string | null, knowledgeBaseName?: string | null) {
      ...
      if (existingIndex >= 0) {
        ...
        if (knowledgeBasePath !== undefined) {
          this.recentFiles[existingIndex].knowledgeBasePath = knowledgeBasePath;
          this.recentFiles[existingIndex].knowledgeBaseName = knowledgeBaseName || null;
        }
      } else {
        // 添加新文件
    ...
    renameRecentFile(oldPath: string, newPath: string) {
      const existingIndex = this.recentFiles.findIndex((file) => file.path === oldPath);
      if (existingIndex >= 0) {
        const fileName = newPath.split(/[\\/]/).pop() || newPath;
        this.recentFiles[existingIndex].path = newPath;
        this.recentFiles[existingIndex].name = fileName;
