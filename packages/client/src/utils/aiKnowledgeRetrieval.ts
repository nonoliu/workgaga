...
export const tokenizeForKnowledgeSearch = (input: string): string[] => {
  ...
  return Array.from(new Set([...latinTokens, ...chineseTokens, ...segmentedChineseTokens]))
...
const pathMatchesCurrentFile = (relativePath: string, currentFilePath?: string | null): boolean => {
  const currentPath = currentFilePath ? normalizePath(currentFilePath) : '';
  if (!currentPath) return false;
  const normalizedRelativePath = normalizePath(relativePath);
  return currentPath.endsWith(`/${normalizedRelativePath}`) || currentPath.endsWith(normalizedRelativePath);
};

const recentFileBoost = (relativePath: string, recentFilePaths: string[] = []): number => {
  const normalizedRelativePath = normalizePath(relativePath);
  const index = recentFilePaths.findIndex((path) => normalizePath(path).endsWith(`/${normalizedRelativePath}`));
  if (index < 0) return 0;
...
export const retrieveAIKnowledgeSnippets = (
  ...
  return notes
    .map((note) => {
      ...
      const currentBoost = pathMatchesCurrentFile(note.relativePath, options.currentFilePath) ? 10 : 0;
      const recentBoost = recentFileBoost(note.relativePath, options.recentFilePaths);
      const exactTitleBoost = tokens.some(
        (token) => title === token || fileName.replace(/\.(md|markdown)$/i, '') === token,
      )
        ? 8
        : 0;
      const score =
        exactTitleBoost + currentBoost + recentBoost + titleHits * 8 + fileNameHits * 7 + pathHits * 3 + contentHits;
