// CI 环境下核心库被重命名为 workgaga-core，本地开发时为 workgaga
// rspack.config.ts 中的 alias 会确保两种情况下都能正确解析
import Cherry from 'workgaga-core';
import { toJpeg, toPng } from 'html-to-image';
import 'workgaga-core/dist/workgaga.min.css';
...
const getPreviewDom = (exportType) => {
  const workgaga = document.querySelector('.cherry-previewer');
  if (!workgaga) {
    vscode.postMessage({ type: `export-${exportType}`, data: 'export-fail' });
    return null;
  }
  return workgaga;
};
...
const startExport = (exportType) => {
  vscode.postMessage({ type: 'export-start', data: { exportType } });
};

const exportImage = async (exportType) => {
  vscode.postMessage({ type: 'export-progress', data: { message: '正在准备导出内容...', increment: 10 } });
  const workgaga = getPreviewDom(exportType);
  if (!workgaga) return;
  try {
    const isPdf = exportType === 'pdf';
    vscode.postMessage({
      type: 'export-progress',
      data: { message: isPdf ? '正在生成 PDF 页面图片...' : '正在生成 PNG 图片...', increment: 35 },
    });
    const dataUrl = isPdf
      ? await toJpeg(workgaga, {
          backgroundColor: '#ffffff',
          cacheBust: true,
          quality: 0.95,
          pixelRatio: window.devicePixelRatio || 1,
        })
      : await toPng(workgaga, {
          backgroundColor: '#ffffff',
          cacheBust: true,
          pixelRatio: window.devicePixelRatio || 1,
        });
    ...
    vscode.postMessage({
      type: 'export-progress',
      data: { message: isPdf ? 'PDF 内容已生成，正在写入文件...' : 'PNG 已生成，正在写入文件...', increment: 15 },
    });
    vscode.postMessage({ type: `export-${exportType}`, data: { dataUrl, ...size } });
  } catch (error) {
...
const customMenuExport = Cherry.createMenuHook('导出', {
  iconName: 'export',
  subMenuConfig: [
    {
      noIcon: true,
...
const basicConfig = {
  ...
  callback: {
    ...
    beforeImageMounted(srcProp, srcValue) {
      ...
      // TODO: 绝对路径(如windows上D:\GithubDesktop\workgaga\README.md)
 ...
 * [cherry language](https://github.com/Tencent/workgaga/wiki/%E5%A4%9A%E8%AF%AD%E8%A8%80);
 * */
