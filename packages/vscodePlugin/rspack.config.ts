...
const webviewConfig: Configuration = {
  ...
  resolve: {
    ...
    // CI 脚本将核心库重命名为 workgaga-core，本地开发时包名仍为 workgaga
    // 此 alias 确保两种场景下 import from 'workgaga-core' 都能正确解析
    // 注意：rspack 需要显式指定入口文件路径，不能仅指定目录
    // __dirname = packages/vscodePlugin，目标在 packages/workgaga/dist/
    alias: {
      'workgaga-core$': path.resolve(__dirname, '../workgaga/dist/workgaga.esm.js'),
      'workgaga-core/dist/workgaga.min.css': path.resolve(
        __dirname,
        '../workgaga/dist/workgaga.min.css'
      ),
    },
  },
  module: {
