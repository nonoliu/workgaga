...
export const uploadFileHandler = async (fileInfo: FileInfo): Promise<UploadFileHandlerRes> => {
  ...
  const uploadType = vscode.workspace.getConfiguration('workgaga').get<UploadType>('UploadType');
  const res: UploadFileHandlerRes = { name, url: '' };
  const backfillImageProps = vscode.workspace
    .getConfiguration('workgaga')
  ...
  switch (uploadType) {
    case 'CustomUploader': {
      const customUploader = vscode.workspace.getConfiguration('workgaga').get<CustomUploader>('CustomUploader');
