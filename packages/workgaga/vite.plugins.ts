 ...
 *
 * 功能：
 * 1. 拦截对 dist/workgaga.js 的请求，重定向到虚拟模块（源码入口 index.js）
 * 2. 拦截对 dist/workgaga.core.js 的请求，重定向到虚拟模块（源码入口 index.core.js）
 * 3. 拦截对 dist/workgaga.css 的请求，重定向到虚拟模块（SCSS 源文件）
 ...
 */
export function cherryDevPlugin(srcDir: string, workgagaDir: string): Plugin {
  // 虚拟模块 ID 前缀
  ...
  function extractAddonFileName(url: string): string | null {
    const match = url.match(/\/packages\/workgaga\/dist\/addons\/([^?]+\.js)/);
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use((req, res, next) => {
        ...
        // 必须在主包匹配之前，避免被 workgaga*.js 的正则吃掉
        ...
        // 2. 拦截 workgaga.core.js 请求 → core 虚拟模块
        // 必须在通用的 workgaga*.js 匹配之前
        const coreJsPattern = /\/?\.{0,2}\/?packages\/workgaga\/dist\/workgaga\.core[^/]*\.js/;
        if (coreJsPattern.test(url)) {
          req.url = `/@id/${virtualCherryCoreJsId}`;
          return next();
        }

        // 3. 拦截 workgaga.js（非 core）请求 → full 虚拟模块
        const jsPattern = /\/?\.{0,2}\/?packages\/workgaga\/dist\/workgaga[^/]*\.js/;
        const cssPattern = /\/?\.{0,2}\/?packages\/workgaga\/dist\/workgaga[^/]*\.css/;
        ...
        const fontPatterns = [/\/packages\/workgaga\/src\/sass\/fonts\/(.+)/, /^\/fonts\/(ch-icon\.[^?]+)/];
        ...
        for (const pattern of fontPatterns) {
          ...
          if (fontMatch) {
            ...
            const fontPath = path.join(workgagaDir, 'dist', 'fonts', fontFile);
...
`;
    ...
    // 2. workgaga*.js → type="module"（虚拟模块是 ES module）
    ...
    transformIndexHtml(html) {
      ...
      result = result.replace(
        /<link[^>]*href=["']([^"']*\/packages\/workgaga\/dist\/workgaga[^"']*\.css)["'][^>]*\/?>/gi,
        (_match, href) => {
          return `<script type="module">import "${href}";</script>`;
        },
      );

      // 2. workgaga*.js（主包和 core）→ type="module"
      result = result.replace(
        /<script([^>]*)\s+src=["']([^"']*\/packages\/workgaga\/dist\/workgaga[^"']*)["']([^>]*)><\/script>/gi,
      ...
      result = result.replace(
        /<script([^>]*)\s+src=["']([^"']*\/packages\/workgaga\/dist\/addons\/[^"']*\.js)["']([^>]*)><\/script>/gi,
        (match, before, src, after) => {
          if (/type\s*=\s*["']module["']/i.test(before + after)) {
            return match;
          }
