...
const getReadyToExport = (previewDom, cb) => {
  const cherryPreviewer = /** @type {HTMLElement}*/ (
    previewDom.cloneNode(true)
  );
  // 当预览区被隐藏时，cherryPreviewer会有cherry-previewer--hidden类，此行用于恢复预览区
  cherryPreviewer.className = cherryPreviewer.className.replace(
...
/**
 * 将预览 DOM 按浏览器实际渲染结果导出为 A4 PDF。
 * @param {HTMLElement} previewDom 预览区域的 dom
 * @param {String} fileName 导出 PDF 文件名
 * @returns {Promise<Uint8Array>}
 */
export async function exportPDF(previewDom, fileName = "") {
  const images = Array.from(previewDom.querySelectorAll("img"));
  await Promise.all(
  ...
  const cssWidth = previewDom.clientWidth;
  const cssHeight = Math.max(previewDom.scrollHeight, previewDom.offsetHeight);
  if (!cssWidth || !cssHeight) {
    const emptyPdf = await PDFDocument.create();
    emptyPdf.addPage([595.28, 841.89]);
    return new Uint8Array(await emptyPdf.save());
  }

  const computedBackgroundColor =
    window.getComputedStyle(previewDom).backgroundColor;
  const isTransparentBackground =
    computedBackgroundColor === "transparent" ||
    /(?:rgba?|hsla?)\([^)]*(?:,|\/)\s*0(?:\.0+)?%?\s*\)$/.test(
      computedBackgroundColor,
    );
  const canvas = await html2canvas(previewDom, {
    allowTaint: true,
    backgroundColor: isTransparentBackground ? "#fff" : computedBackgroundColor,
    height: cssHeight,
    width: cssWidth,
    scrollY: 0,
    scrollX: 0,
  ...
  const renderScaleX = canvas.width / cssWidth;
  const renderScaleY = canvas.height / cssHeight;
  const pageWidth = 595.28;
  const pageHeight = 841.89;
  const pointsPerCssPixel = pageWidth / cssWidth;
  const pageCssHeight = pageHeight / pointsPerCssPixel;
  const rootRect = previewDom.getBoundingClientRect();
  const rootContentTop =
    rootRect.top + previewDom.clientTop - previewDom.scrollTop;
  const avoidBreakElements = Array.from(previewDom.querySelectorAll("*"))
    .map((element) => {
      ...
      if (
        breakInside !== "avoid" &&
        !/^(h[1-6]|pre|code|table|img|figure|blockquote)$/.test(tagName)
      )
        return null;
      const rect = element.getBoundingClientRect();
      return {
        bottom: rect.bottom - rootContentTop,
        top: rect.top - rootContentTop,
      };
    })
    .filter(
      (item) =>
        ...
        item.top >= 0 &&
        item.bottom <= cssHeight,
    );

  const getSliceEnd = (sliceTop) => {
    const desiredEnd = Math.min(cssHeight, sliceTop + pageCssHeight);
    if (desiredEnd === cssHeight) return desiredEnd;
    const crossing = avoidBreakElements.filter(
      ({ top, bottom }) =>
        top < desiredEnd &&
        bottom > desiredEnd &&
        bottom - top <= pageCssHeight,
    );
    if (!crossing.length) return desiredEnd;
    const movedEnd = Math.min(...crossing.map(({ top }) => top));
    return movedEnd > sliceTop ? movedEnd : desiredEnd;
  };

  const pdfDoc = await PDFDocument.create();
  let sliceTop = 0;
  while (sliceTop < cssHeight) {
    let sliceEnd = getSliceEnd(sliceTop);
    if (sliceEnd <= sliceTop)
      sliceEnd = Math.min(cssHeight, sliceTop + pageCssHeight);
    const sliceHeight = sliceEnd - sliceTop;
    const sliceCanvas = document.createElement("canvas");
    sliceCanvas.width = Math.max(1, Math.round(cssWidth * renderScaleX));
    sliceCanvas.height = Math.max(1, Math.round(sliceHeight * renderScaleY));
    const sourceTop = Math.min(
      canvas.height,
      Math.round(sliceTop * renderScaleY),
    );
    const sourceBottom = Math.min(
      canvas.height,
      Math.round(sliceEnd * renderScaleY),
    );
    ...
    const renderedSliceHeight = sourceHeight / renderScaleY;
    const renderedSliceHeightInPoints = renderedSliceHeight * pointsPerCssPixel;
    const page = pdfDoc.addPage([pageWidth, pageHeight]);
    page.drawImage(png, {
      x: 0,
      y: pageHeight - renderedSliceHeightInPoints,
      width: pageWidth,
      height: renderedSliceHeightInPoints,
    });
    sliceTop = sliceEnd;
  }
...
export function exportScreenShot(previewDom, fileName) {
  getReadyToExport(
    previewDom,
    (
      /** @type {HTMLElement}*/ cherryPreviewer,
      /** @type {function}*/ thenFinish,
    ) => {
      window.scrollTo(0, 0);
      // 去掉audio和video标签
      ...
      html2canvas(cherryPreviewer, {
        ...
        ignoreElements: (element) => {
          if (
            cherryPreviewer === element ||
            cherryPreviewer.contains(element) ||
            element.contains(cherryPreviewer)
          ) {
            return false;
          }
...
export function canvas2img(dom, fileName, options = {}) {
  ...
  const bg =
    format === "png"
      ? "transparent"
      : getComputedStyle(previewer).backgroundColor;
