...
// workgaga 源码目录
const workgagaDir = path.resolve(__dirname);
const examplesDir = path.resolve(__dirname, '../../examples');
const srcDir = path.resolve(workgagaDir, 'src');
...
export default defineConfig({
  ...
  server: {
    ...
    fs: {
      ...
      allow: [
        examplesDir, // 开发示例
        path.resolve(workgagaDir, 'src'), // 源码
        path.resolve(workgagaDir, 'dist/fonts'), // 字体文件
      ],
      // 显式禁止访问敏感目录（Vite 4.3.9+ 支持）
      deny: [
        path.resolve(workgagaDir, '.env'),
        path.resolve(workgagaDir, '.env.local'),
        path.resolve(workgagaDir, '.env.*.local'),
      ],
    },
  },
  ...
  plugins: [cherryDevPlugin(srcDir, workgagaDir), printLinksPlugin(examplesDir, htmlPages)],
});
